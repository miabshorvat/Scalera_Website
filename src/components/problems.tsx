      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mt-20 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {t.problems.items.map((problem, i) => (
            <div
              key={i}
              className="glass glass-hover group relative rounded-2xl p-8 transition-all duration-300"
            >
              <div className="absolute right-6 top-6 text-5xl font-bold text-white/[0.03]">
                {String(i + 1).padStart(2, "0")}
              </div>
              <div className="relative">
                <svg className="mb-4 h-5 w-5 text-accent-500/40" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="mt-16 text-center">
          <Link
            href="/kontakt"
            className="group inline-flex items-center gap-2 text-lg font-semibold text-accent-400 transition-colors hover:text-accent-300"
          >
            {t.problems.cta}
            <svg
